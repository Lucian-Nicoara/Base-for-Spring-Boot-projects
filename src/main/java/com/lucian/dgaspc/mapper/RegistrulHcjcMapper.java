...
public interface RegistrulHcjcMapper {
	...
	@Select("select id, nrHotarare, DATE_FORMAT(dataHotarare, '%d.%m.%Y') as dataHotarare, emitent, DATE_FORMAT(dataComunicarii, '%d.%m.%Y') as dataComunicarii, idCompartiment, compartimentulAdresat, idPersoanaPrimire, persoanaPrimire, DATE_FORMAT(dataPrimirii, '%d.%m.%Y') as dataPrimirii, numeFisier, stadiu from registrul_hcjc where sters is null order by id desc")
	List<HotarareCJC> getHotarariCJC();
	
	@Select("select id, nrHotarare, DATE_FORMAT(dataHotarare, '%d.%m.%Y') as dataHotarare, emitent, DATE_FORMAT(dataComunicarii, '%d.%m.%Y') as dataComunicarii, idCompartiment, , idPersoanaPrimire, persoanaPrimire, DATE_FORMAT(dataPrimirii, '%d.%m.%Y') as dataPrimirii, numeFisier, stadiu from registrul_hcjc where id = #{id} and sters is null")
	...
	@Select("select id, nrHotarare, DATE_FORMAT(dataHotarare, '%d.%m.%Y') as dataHotarare, emitent, DATE_FORMAT(dataComunicarii, '%d.%m.%Y') as dataComunicarii, idCompartiment, compartimentulAdresat, idPersoanaPrimire, persoanaPrimire, DATE_FORMAT(dataPrimirii, '%d.%m.%Y') as dataPrimirii, numeFisier, fisier, stadiu from registrul_hcjc where id = #{id} and sters is null ")
